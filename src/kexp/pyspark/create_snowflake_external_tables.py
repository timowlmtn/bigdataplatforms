...
def main():
    ...
    source = "kexp"
    delta_schema_array = ["bronze", "silver"]
    catalog = spark_catalog.SparkCatalog(source_name=source,
                                         lake_location=f'{os.getenv("DELTA_LAKE_FOLDER")}/{source}')

    for delta_schema in delta_schema_array:
        print(f'Listing: {os.getenv("DELTA_LAKE_FOLDER")}/{source}/{delta_schema}')
        for file in os.listdir(f'{os.getenv("DELTA_LAKE_FOLDER")}/{source}/{delta_schema}'):
            with open(f'../data/sql/snowflake_create_ext_{delta_schema}_{file.lower()}.sql', "w") as sql_file_out:
                print(f'Writing: ../data/sql/snowflake_create_ext_{delta_schema}_{file.lower()}.sql')
                ...
                data_frame_schema = catalog.get_schema_json(delta_schema, file)

                sql_file_out.write(
                    snowflake_delta.generate_external_table_ddl(table_name=f'{os.getenv("SNOWFLAKE_LANDING_ZONE")}.'
